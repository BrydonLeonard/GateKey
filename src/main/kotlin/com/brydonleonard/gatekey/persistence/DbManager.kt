...
class DbManager(
    ...
    fun applyMigrations(nonMigrationDaos: List<Dao<*, *>>, newTables: List<String>) {
        ...
        logger.info {
            "DB is at version $currentVersion. Current latest version is $latestVersion. " +
                    "Created ${newTables.size} new tables of ${nonMigrationDaos.size} total tables"
        }

        if (currentVersion < latestVersion) {
            dbMigrations.subList(currentVersion + 1, latestVersion + 1).forEachIndexed { version, migration ->
